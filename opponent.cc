...
void Opponent::Draw(graphics::Image &background) {
  ...
  if (x_rate_ < 0 && y_rate_ > 0) {
      Helper(background, oaFrames[0].get(), x, y);
  } else if ( x_rate_ < 0 && y_rate_ < 0) {
     Helper(background, oaFrames[frame].get(), x, y);
  } else if ( x_rate_ > 0 && y_rate_ > 0) {
     rHelper(background, oaFrames[0].get(), x, y);
  } else if ( x_rate_ > 0 && y_rate_ < 0) {
     rHelper(background, oaFrames[frame].get(), x, y);
  }
